...
if old_route in s:
    ...
    # Normalize any current diagnostic wrapper into the cleanup-aware form.
    marker = "def _diagnostic_route(self, decision):\n"
    if marker in s and "self.keyboard.release_all()" in s and "self.arm.flush()" in s:
        start = s.index(marker)
        end = s.index("runtime.Combined.route = _diagnostic_route", start)
        wrapper = r'''def _diagnostic_route(self, decision):
...
'''
    ...
    elif "CLEANUP/keyboard" not in s:
