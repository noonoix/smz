        ...
        if op in ("RUNFOR", "AUTORESUME", "POSTLAUNCH"):
            ...
            if op == "RUNFOR":
                ...
                if len(p) != 6 or p[0] not in ("0", "1"):
                    raise ValueError("line %d: POSTLAUNCH needs 0|1,slot,beforeMin,beforeMax,afterMin,afterMax" % line_no)
                try:
                    slot = int(p[1])
                except Exception:
...
def _launch_pinned_app(ctx, rng, policy):
    """Common launch preamble used on initial start and Auto Resume."""
...
def _run_launch_steps(ctx, path="/launch_steps.txt"):
    ...
    if not text.lstrip().startswith("PLAN|2"):
        raise ValueError("launch_steps.txt must start with PLAN|2")
    ops = plan_engine.parse_plan(text)
    ctx.log("cycle: Launch Steps start")
    plan_engine.run_plan(ops, ctx)
    ctx.log("cycle: Launch Steps complete")
    return 1
