...
public static class PipelinePlanBundle
{
        ...
        AppSettings settings, int screenW, int screenH, string sourceName, string machine)
    {
        ...
        // AutoCyclePlanBundle owns root directives; only expand the two explicit call sentinels.
    ...
    private static void ValidateRecoveryCalls(PipelineWorkspace workspace)
    {
        ...
        foreach (var tab in workspace.Tabs)
            Visit(tab.Steps, tab.Kind, tab.Title, errors);
        ...
        static void Visit(IEnumerable<StepNode> nodes, PipelineKind kind, string title, List<string> errors)
        {
            ...
            {
                if (node.Type == RecoveryCallStepDefinitions.CallMain && kind != PipelineKind.Main)
                    errors.Add(title + ": استپ Run/Call Main DC Recovery فقط در تب Main مجاز است.");
                if (node.Type == RecoveryCallStepDefinitions.CallLaunch && kind != PipelineKind.Launch)
                    errors.Add(title + ": استپ Run/Call Launch DC Recovery فقط در تب Launch مجاز است.");
    ...
    /// <summary>
    /// The stock compiler already knows comments and INCLUDE. A typed call is normalized to an
    /// unambiguous private comment and expanded after compilation, preserving If/Else nesting.
    /// </summary>
