...
combined_ui = (root / 'ams-shell/src/Ams.UI/MainWindow.CombinedGuardExportUi.cs').read_text(encoding='utf-8')
legacy_ui = (root / 'ams-shell/src/Ams.UI/MainWindow.AutoCycleFirmwareExportUi.cs').read_text(encoding='utf-8')
...
assert 'ExportAutoCyclePicoFirmware' in vm
assert 'ExportCombinedPortableGuardCommand' in combined_ui
assert 'ExportAutoCyclePicoFirmwareCommand' not in legacy_ui
...
print('auto-cycle firmware integration + combined export contract: 26 passed, 0 failed')
