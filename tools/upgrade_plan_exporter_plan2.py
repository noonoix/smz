...
if 'PLAN2_PARITY_MIGRATION' in s:
    print('PLAN2 parity migration already applied')
    raise SystemExit(0)
s = s.replace('namespace Ams.UI.Services;','namespace Ams.UI.Services;\n\n// PLAN2_PARITY_MIGRATION')

def method_span(src, signature):
    a = src.index(signature); b = src.index('{', a); depth = 0
...
def replace_method(src, signature, body):
    a,b = method_span(src, signature); return src[:a] + body.rstrip() + src[b:]

s = s.replace('private readonly Dictionary<StepNode, string> _numbering = new();','''private readonly Dictionary<StepNode, string> _numbering = new();
        private readonly HashSet<string> _labels = new(StringComparer.Ordinal);
        private readonly string _sourcePath;''')
s = s.replace('public Gen(AppSettings settings) => _settings = settings;','''public Gen(AppSettings settings, string sourcePath)
        { _settings = settings; _sourcePath = sourcePath; }''')
s = s.replace('var gen = new Gen(settings);', 'var gen = new Gen(settings, sourceName);')

s = replace_method(s, '        private int EmitIf(', r'''        private int EmitIf(IList<StepNode> nodes, int i)
        {
...
s = replace_method(s, '        private void EmitStep(', r'''        private void EmitStep(StepNode n)
        {
            switch(n.Type)
            {
...
'''
s=s.replace('        private void EmitForLoop(StepNode n)',extra+'\n        private void EmitForLoop(StepNode n)')
s=replace_method(s,'        private void EmitMouseMove(',r'''        private void EmitMouseMove(StepNode n){int x=PropEx.GetInt(n.Props,"x",600),y=PropEx.GetInt(n.Props,"y",497);if(!PropEx.GetBool(n.Props,"human",true)){Emit(n,new[]{"MOVETO|x="+x+"|y="+y+"|human=0"},"MOVETO");return;}Emit(n,new[]{"MOVETO|x="+x+"|y="+y+Tuning(n,(1,1,0,0))},"MOVETO");}''')
s=replace_method(s,'        private void EmitWaitForLight(',r'''        private void EmitWaitForLight(StepNode n){var p=n.Props;var(lo,hi,stable,to,mode)=LuxArgs(p);var head="WLIGHT|"+lo+","+hi+","+stable+","+to+","+mode;if(PropEx.GetBool(p,"armed")){var vk=Vk(n,PropEx.GetString(p,"key","E"),"waitForLight armed key");if(vk==0)return;var(h0,h1)=Pair(PropEx.GetInt(p,"holdMin",30),PropEx.GetInt(p,"holdMax",90));var(r0,r1)=Pair(PropEx.GetInt(p,"reactMin",80),PropEx.GetInt(p,"reactMax",180));head+="|key="+vk+","+h0+","+h1+"|react="+r0+","+r1;}Emit(n,new[]{head},"WLIGHT");}''')
...
validator=r'''    private static void ValidatePlan(string text)
    {
        ...
        var lines=text.Split('\n');for(int li=0;li<lines.Length;li++){var line=lines[li].Trim();if(line.Length==0||line.StartsWith('#'))continue;var f=line.Split('|');var op=f[0];void Bad(string why)=>throw new PlanBlockedException(new[]{"PlanExporter BUG: line "+(li+1)+": "+why+" (please report)"});if(!known.Contains(op))Bad("unknown op '"+op+"'");if(first&&(op!="PLAN"||f.Length<2||f[1]!="2"))Bad("PLAN must be first with version 2");if(op=="PLAN"&&!first)Bad("duplicate/non-first PLAN");if(op is "LOOP" or "LOOPTIME")loops++;else if(op=="ENDLOOP"&&--loops<0)Bad("ENDLOOP without LOOP");if(op is "IFSND" or "IFLUX")ifs++;else if(op=="ELSE"&&ifs<=0)Bad("ELSE without IF");else if(op=="ENDIF"&&--ifs<0)Bad("ENDIF without IF");if(op=="RPKG")packages++;else if(op=="PKGITEM"&&packages<=0)Bad("PKGITEM outside RPKG");else if(op=="ENDPKG"&&--packages<0)Bad("ENDPKG without RPKG");if(op=="PGROUP")groups++;else if(op=="PARITEM"&&groups<=0)Bad("PARITEM outside PGROUP");else if(op=="ENDPAR"&&--groups<0)Bad("ENDPAR without PGROUP");if(op=="LABEL"){if(f.Length<2||f[1].Length==0||!labels.Add(f[1]))Bad("bad/duplicate LABEL");}if(op=="GOTO"){if(f.Length<2||f[1].Length==0)Bad("empty GOTO");gotos.Add(f[1]);}if(op=="INCLUDE"&&(!HasKv(f,"file",out var file)||file.Length==0||file.Any(ch=>ch<32||ch>126||"/\\:|%".Contains(ch))))Bad("unsafe INCLUDE filename");first=false;}
        if(loops!=0||ifs!=0||packages!=0||groups!=0)throw new PlanBlockedException(new[]{"PlanExporter BUG: unclosed PLAN|2 container (please report)"});foreach(var g in gotos)if(!labels.Contains(g))throw new PlanBlockedException(new[]{"PlanExporter BUG: GOTO target '"+g+"' is undefined (please report)"});
    }'''
