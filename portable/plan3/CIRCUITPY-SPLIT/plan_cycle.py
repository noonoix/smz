...
The shared launch preamble (random wait, Win+slot, ordered Launch Steps) runs
once on both initial start and a genuine Auto Resume.
"""
import random
import time
...
class _CycleContext:
    def __init__(self, inner, cycle, essentials=None):
        object.__setattr__(self, "_inner", inner); object.__setattr__(self, "_cycle", cycle); object.__setattr__(self, "_essentials", essentials)
    def __getattr__(self, name): return getattr(self._inner, name)
    def __setattr__(self, name, value):
        if name in ("_inner", "_cycle", "_essentials"): object.__setattr__(self, name, value)
...
def _run_launch_steps(ctx, path="/launch_steps.txt"):
    ...
    ops = plan_engine.parse_plan(text); ctx.log("cycle: Launch Steps start"); plan_engine.run_plan(ops, ctx); ctx.log("cycle: Launch Steps complete"); return 1
...
def run_root(text, ctx, rng=None, arm_store=None):
    ...
    cycle.configure(policy["run"][0], policy["run"][1], policy["auto"], policy["resume"][0], policy["resume"][1]); cycle.start()
    essentials = getattr(ctx, "resume_essentials", None); wrapped = _CycleContext(ctx, cycle, essentials)
    try:
        _launch_pinned_app(wrapped, rng or random, policy["launch"]); _run_launch_steps(wrapped)
