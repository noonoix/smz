...
public sealed class RunEngine
{
    ...
    /// <summary>v0.9.68 — run-scoped explicit key state, isolated per keyboard route.
    ...
    public async Task RunAsync(IEnumerable<StepNode> roots, CancellationToken ct)
    {
        ...
        lock (_heldKeyboardLock) _heldKeyboardKeys.Clear();
                ...
                default:
                {
                    ...
                    foreach (var cmd in StepDefinitions.GetCommands(s))
                    {
                        ...
                        else
                        {
                            ...
                            if (TryHandleKeyboardStateGuard(wireCmd, out var guardedCmd, out var guardLog))
                            {
    ...
    private bool TryHandleKeyboardStateGuard(string cmd, out string? guardedCmd, out string? guardLog)
    {
        guardedCmd = cmd;
        guardLog = null;
        string body = cmd;
