...
/// <summary>Applies the AutoCycle patch and publishes the complete runtime transactionally.</summary>
public static class AutoCycleFirmwareBundle
{
    ...
    private static readonly string[] RuntimeFiles =
    {
        ...
        "auto_resume_boot.py", "resume_essentials_runtime.py", "recovery_runtime.py",
    };
        ...
        string machine, string loopMode, int loopCount, int loopSeconds, bool keyboardOnArm)
    {
        ...
        foreach (var name in RuntimeFiles.Append(PatchManifest))
            if (!File.Exists(Path.Combine(runtimeDir, name))) throw new IOException("فایل runtime چرخه پیدا نشد: " + name);
    ...
    {
        code = code.Replace("\r\n", "\n").Replace('\r', '\n');
        var manifest = JsonSerializer.Deserialize<PatchDocument>(File.ReadAllText(manifestPath), new JsonSerializerOptions { PropertyNameCaseInsensitive = true })
        ...
        foreach (var marker in RequiredMarkers)
            if (!code.Contains(marker, StringComparison.Ordinal)) throw new InvalidDataException("پست‌کاندیشن Firmware چرخه پیدا نشد: " + marker);
        return code;
    }
    ...
    private static string ReplaceOnce(string text, string oldText, string newText)
    {
        ...
        if (first < 0 || text.IndexOf(oldText, first + oldText.Length, StringComparison.Ordinal) >= 0) throw new InvalidDataException("قالب firmware با قرارداد چرخه همگام نیست: " + oldText.Split('\n')[0]);
    ...
    {
        var tx=Guid.NewGuid().ToString("N"); var temps=payloads.Select(p=>p.Path+"."+tx+".tmp").ToArray(); var backups=payloads.Select(p=>p.Path+"."+tx+".bak").ToArray(); var published=new List<int>();
        try { for(var i=0;i<payloads.Count;i++)File.WriteAllBytes(temps[i],payloads[i].Bytes); for(var i=0;i<payloads.Count;i++){if(File.Exists(payloads[i].Path))File.Move(payloads[i].Path,backups[i]);File.Move(temps[i],payloads[i].Path);published.Add(i);} foreach(var backup in backups)if(File.Exists(backup))File.Delete(backup); }
        catch { foreach(var i in published.AsEnumerable().Reverse())if(File.Exists(payloads[i].Path))File.Delete(payloads[i].Path); for(var i=0;i<payloads.Count;i++)if(File.Exists(backups[i]))File.Move(backups[i],payloads[i].Path,true); throw; }
        finally { foreach(var file in temps.Concat(backups))if(File.Exists(file))File.Delete(file); }
    }
}
