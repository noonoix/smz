# Phase 7 — Guard transition matrix

This document records the approved relation between the six optical profiles and the Classroom Studio pipeline tabs. It defines the transition policy; it does not by itself enable RunEngine execution.
...
A stable optical match may advance the controller only to the next stage in this table. It must not skip a stage or execute the same transition repeatedly while the optical state remains unchanged.

`Resumable` remains a separate workspace and is not part of the optical progression.

## DC fallback

`login-or-dc` is one optical calibration profile, not two profiles. The application/pipeline context distinguishes Login from DC.

When the Guard observes the DC optical signature at any active in-game point, it must:

1. leave the current progression context;
2. fall back to stage 2 (`Login / DC`);
3. select the independent `DC` Classroom Studio context;
...
While the controller is in the game context, a stable Targeted match may preempt the current game observation:
...
If the DC signature is detected while Targeted is active, DC has priority and the controller falls back to stage 2 instead of completing the Targeted side-state first.

## Execution guards

The future Guard-to-RunEngine adapter must enforce all of the following:
...
- no execution from duplicate Guard events or from a workspace mutation that was not revalidated.

The seven visible tabs and this matrix do not authorize automatic execution until those boundaries are implemented and covered by tests. Physical hardware acceptance remains separate.
