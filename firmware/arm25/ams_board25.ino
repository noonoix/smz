...
static void handle(char* cmd) {
  ...
  if (!strcmp(cmd, "VER"))     {
    char b[80];
    // fw 2.3: the |FRM capability marker is advertised ONLY on the brain link
    // (g_out == &Serial1); the encrypted USB reply stays byte-identical to fw 2.2
    // so the PC-side bridge parser is untouched.
