...
try:
 import microcontroller; _NVM=microcontroller.nvm
...
class ResumeArmStore:
 ...
 def try_arm_restart(self,maximum=DEFAULT_MAX_RESTARTS,auto_resume=True):
  ...
  if count>=max(0,int(maximum)): self.clear(); return False
...
class RootCycle:
 ...
 def configure(self,run_min_seconds,run_max_seconds,auto_resume_enabled,resume_min_seconds,resume_max_seconds):
  ...
  self.run_range=self._positive_range(run_min_seconds,run_max_seconds,"RUNFOR"); self.resume_range=self._positive_range(resume_min_seconds,resume_max_seconds,"AUTORESUME"); self.auto_resume_enabled=bool(auto_resume_enabled)
 def start(self):
